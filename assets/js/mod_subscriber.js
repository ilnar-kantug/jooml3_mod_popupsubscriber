...
if( jQuery.cookie('pop_up_subs_form_cookie') != 'doNotShow'){
	...
	jQuery(document).ready(function(){
		var pop_up_counter = jQuery('#jsData').data('counter');
		var sending_error = jQuery('#jsData').data('sending-error');
		var language_tag = [jQuery('#jsData').data('language')];
		...
		if(Number(jQuery.cookie('pop_up_subs_form_cookie')) < Number(pop_up_counter)){
		...
		setTimeout(function() {
			...
			var popUpWidth = jQuery('#pop_up_subs_form_wrapper').outerWidth();
			var popUpHeight = jQuery('#pop_up_subs_form_wrapper').height();
			jQuery('#pop_up_subs_form_wrapper').outerWidth(popUpWidth);
			jQuery('#pop_up_subs_form_wrapper').height(popUpHeight);
		}, 3000);
		...
		jQuery('#subscriber-form').submit(function(e){
			...
			//serialize form
			var subscriberData = jQuery('#subscriber-form').serialize()+'&language='+language_tag;
			//Submit Form
			jQuery.ajax({
				type: 'post',
				...
				if(data.responseText !== ''){
					...
					jQuery('#form-msg').text(sending_error);
				}
			});
